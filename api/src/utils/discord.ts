...
type CheckPermissionsReturn = Promise<PromiseTuple<boolean>>;
...
export async function getTextChannel(channelId: string): TextChannelReturn {
    try {
        const channel = await bot.channels.fetch(channelId) as Channel;
...
export async function getGuild(guildId: GuildResolvable): Promise<PromiseTuple<Guild>> {
    const guildPromise = bot.guilds.fetch({
        guild: guildId
    });
...
export async function getGuildMember({ guildId, discordId }: GuildMemberOptions): Promise<PromiseTuple<GuildMember>> {
...
export async function isGuildMember({ guildId, discordId }: GuildMemberOptions): Promise<PromiseTuple<boolean>> {
...
export async function hasRole({ guildId, discordId, roles }: HasRoleOptions): Promise<PromiseTuple<boolean>> {
    const [guildMember, err] = await getGuildMember({ guildId, discordId });
